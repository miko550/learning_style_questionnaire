...
const AdminDashboard = () => {
  ...
  const changePassword = async (e) => {
    ...
    try {
      setChanging(true);
      await axios.post('/admin/change-password', passwords);
  ...
  const downloadCsv = async () => {
    try {
      const resp = await axios.get('/admin/export-results', { responseType: 'blob' });
    ...
    <div>
      <div className="card">
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          <button className="btn" onClick={downloadCsv}>Download results (CSV)</button>
          <form onSubmit={changePassword} style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            <input type="password" placeholder="New password" value={passwords.new_password} onChange={(e)=>setPasswords({...passwords, new_password: e.target.value})} required />
            <button className="btn btn-secondary" type="submit" disabled={changing}>{changing ? 'Saving...' : 'Change password'}</button>
          </form>
        </div>
      </div>
